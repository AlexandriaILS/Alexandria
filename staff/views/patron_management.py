...
from staff.forms import PatronForm, PatronEditForm
from users.models import AlexandriaUser, USLocation
...
def create_patron(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            newuser = AlexandriaUser.objects.create_user(
    ...
    def get(self, request, user_id):
        if request.user.is_superuser:
            ...
            user = get_object_or_404(
                AlexandriaUser, card_number=user_id, host=request.get_host()
            )
