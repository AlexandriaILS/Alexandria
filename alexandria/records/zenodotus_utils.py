...
def slash_join(*args: str, params: Dict = None) -> str:
    ...
    result = "/".join(str(arg).strip("/") for arg in args) + "/"
    if params:
        params = f"?" + "&".join(
            [
                *list(
...
def get_base_id(route, level) -> [int, None]:
    resp = requests.get(slash_join(settings.ZENODOTUS_URL, route))
...
def sync_object_with_z(object, endpoint, itemtype=False) -> int:
    ...
    # the ID.
    resp = requests.get(
        slash_join(settings.ZENODOTUS_URL, endpoint, params={"q": object.name})
    )
    resp.raise_for_status()
    result = resp.json()
    if len(result) > 0:
        ...
        resp = requests.post(slash_join(settings.ZENODOTUS_URL, endpoint), data=data)
        resp.raise_for_status()
...
def upload(record):
    ...
    if not record.zenodotus_id:
        ...
        result = requests.get(
            slash_join(settings.ZENODOTUS_URL, "record", params=params)
        )
        ...
        if len(result) == 0:
            ...
            resp = requests.post(
                slash_join(settings.ZENODOTUS_URL, "record"), data=data, files=files
            )
            resp.raise_for_status()
            return True
