...
def _get_user(base_data: Dict, **kwargs) -> AlexandriaUser:
    user_info = {
        **base_data,
        **{key: kwargs[key] for key in kwargs if key in dir(AlexandriaUser)},
    }
    user, _ = AlexandriaUser.objects.get_or_create(**user_info)
    return user
